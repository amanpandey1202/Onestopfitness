...
export default function TrainerError({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gym-black px-4 text-center text-white">
      ...
      <p className="mt-3 max-w-md text-sm text-white/60">Please try again, or return to your dashboard.</p>
      <div className="mt-8 flex gap-3">
        <button onClick={reset} className="rounded-md bg-gym-lime px-6 py-2 text-sm font-bold text-gym-black">
          Try again
        </button>
        <Link href="/trainer" className="rounded-md border border-white/20 px-6 py-2 text-sm font-bold text-white">
          Dashboard
        </Link>
      </div>
    </div>
